...
//       showMessage(".hit"); // hit on first white ring
//     } else {
//       onMiss(); // too far → miss
//     }
//   }
// }
function openPopup() {
  document.getElementById("bullseyePopup").style.display = "grid";

}

function closePopup() {
  document.getElementById("bullseyePopup").style.display = "none";
}
function hitTest(tween) {
  // check for collisions with arrow and target
  var arrow = tween.target[0];
  ...
  if (intersection.segment1 && intersection.segment2) {
    ...
    if (distance < 7) {
      tween.pause();  // stop arrow on bullseye
      showMessage(".bullseye");
	  openPopup(); 
	  window.parent.postMessage("bullseye-done", "*");
    } else {
      // hit, but not bullseye → still a miss
      onMiss();
    }
  }
}
