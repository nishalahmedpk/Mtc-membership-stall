...
function make_bird_slow_and_fall() {
  if (game_mode === 'win') return; // ignore gravity in win state
...
function add_pipe(x_pos, top_of_gap, gap_width) {
  // Difficulty control
  var min_gap = 80;   // never smaller than this
  var max_gap = 160;  // optional max size

  // Shrink gap gradually for difficulty
  gap_width -= 2;
  if (gap_width < min_gap) gap_width = min_gap;
  if (gap_width > max_gap) gap_width = max_gap;
...
function display_intro_instructions() {
  ...
  ctx.fillStyle = 'blue';
  ctx.fillText(
    'Reach score ' + targetScore + ' to finish the game!',
    myCanvas.width / 2,
    myCanvas.height / 4 + 40
  );
}
...
function display_game_win() {
  ctx.font = '30px Arial';
  ctx.fillStyle = 'green';
  ctx.textAlign = 'center';
  ctx.fillText('You Win!', myCanvas.width / 2, 100);
  ctx.fillText('Score: ' + score, myCanvas.width / 2, 150);
  ctx.font = '20px Arial';
  ctx.fillText('Click, touch, or press to play again', myCanvas.width / 2, 300);
}
...
function Do_a_Frame() {
  ...
  switch (game_mode) {
    ...
    case 'running': {
      ...
      // Count score when passing top pipes
    ...
    case 'win': {
      ...
      // ✅ Send message to parent only once
      if (!window.flappyReported) {
        window.flappyReported = true;
        window.parent.postMessage("flappy-done", "*");

        // Optional: close iframe after short delay
        setTimeout(() => {
